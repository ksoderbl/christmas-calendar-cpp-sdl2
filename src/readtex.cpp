...
static rawImageRec *RawImageOpen(string fileName)
{
   union {
      int testWord;
      char testByte[4];
   ...
   if ((raw->file = fopen(fileName.c_str(), "rb")) == 0) {
   ...
   if (raw->tmp == 0 || raw->tmpR == 0 || raw->tmpG == 0 ||
       raw->tmpB == 0) {
      cerr << "Out of memory!\n";
      return 0;
   }
   ...
   if ((raw->type & 0xFF00) == 0x0100) {
      ...
      if (raw->rowStart == 0 || raw->rowSize == 0) {
	 cerr << "Out of memory!\n";
         return 0;
      }
      raw->rleEnd = 512 + (2 * x);
...
static TK_RGBImageRec *tkRGBImageLoad(string fileName)
{
   rawImageRec *raw;
 ...
 */
GLboolean LoadRGBMipmaps( string imageFile, GLint intFormat )
{
   GLint error;
   GLenum format;
   ...
   if (image->components==3) {
      ...
      /* not implemented */
      cerr << "Error in LoadRGBMipmaps "
	   << image->components << "-component images not implemented\n";
      return GL_FALSE;
   }
 ...
 * Return:  pointer to image data or 0 if error
 */
GLubyte *LoadRGBImage( string imageFile, GLint *width, GLint *height,
                       GLenum *format )
{
   TK_RGBImageRec *image;
   GLint bytes;
   ...
   if (image->components==3) {
      ...
      *format = GL_RGBA;
      ...
      /* not implemented */
      cerr << "Error in LoadRGBImage " << image->components
	   << "-component images not implemented\n";
      return 0;
   }
